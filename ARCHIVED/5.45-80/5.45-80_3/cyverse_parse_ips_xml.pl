...
sub parse_xml($$$) {
  ...
  $results = `grep -P "<protein>|<xref|<entry|<go-xref|<pathway-xref|</protein>" $opt_f/$input_file`;
  @res_array = split(/\n/,$results);
   $rcnt=0;
...
sub generate_gaf_output($$$$$) {
   ...
   my ($oname, $gaf_fh, $output_gaf_file, $date, $obj_type,$taxon,$db,$assigned,$aspect,$qual); 
  ...
  foreach my $acc (sort keys %acc_go) {
     ...
     my $comp_fh;
     my $compy = '/opt/interproscan/protein-containing_complex_and_children.txt';
     unless (open $comp_fh, "$compy") {
     print STDERR "Could not open file 'protein-containing_complex_and_children.txt': $!\n";
     # we return 'undefined', we could also 'die' or 'croak'
     return undef
     }
     chomp(my @complex = <$comp_fh>);
     unless (close $comp_fh) {
     # what does it mean if close yields an error and you are just reading?
     print STDERR "Don't care error while closing 'protein-containing_complex_and_children.txt': $!\n";
     }
     ...
     foreach my $go_id (keys %{$acc_go{$acc}}) {
         ...
         if (uc($aspect) eq 'P') { $qual = 'involved_in'; }
         elsif (uc($aspect) eq 'F') { $qual = 'enables'; }
	 elsif (uc($aspect) eq 'C' && ( grep( /^$go_id$/, @complex ) )) { $qual = 'part_of'; } 
	 elsif (uc($aspect) eq 'C' && ( grep -v( /^$go_id$/, @complex ) )) { $qual = 'located_in'; } 
         ...
         print $gaf_fh "$db\t$db_object_id\t$db_object_id\t$qual\t$go_id\tGO_REF:0000002\tIEA\t$with_from\t$aspect\t$db_object_name\t\t$obj_type\ttaxon:$taxon\t$date\t$assigned\t\t\n";
   } # foreach go_ids
} # foreach acc
close $gaf_fh;
