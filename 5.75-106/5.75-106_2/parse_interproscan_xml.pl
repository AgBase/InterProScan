...
&parse_xml ($opt_i,$output_filename,$opt_y,$opt_f);

&parse_mapping_file($opt_m);

&remap_bad_input($opt_b,$opt_y);

&generate_interpro_output($output_filename,$opt_f);
   undef(%acc_interpro);
   undef(%interpro_names);

&generate_go_output($output_filename,$opt_f);

&generate_pathway_output($output_filename,$opt_f);
    undef(%acc_pathway);

&generate_gaf_output($output_filename,$opt_y,$opt_t,$opt_d,$opt_n,$opt_f);
    undef(%acc_go);
    undef(%go_names);
...
sub parse_xml($$$$) {
  ...
  $err_file = $opt_f . '/' . $output_file . '.err';
...
sub generate_interpro_output($$) {
  ...
  $oname=shift;
  $output_interpro_file = $opt_f .'/'. $oname . '_acc_interpro_counts.txt';
 ...
 $output_interpro_file2 = $opt_f .'/'. $oname . '_interpro_counts.txt';
...
sub generate_go_output($$) {
  ...
  $oname=shift;
  $output_go_file = $opt_f .'/'. $oname . '_acc_go_counts.txt';
  ...
  $output_go_file2 = $opt_f .'/'. $oname . '_go_counts.txt';
...
sub generate_pathway_output($$) {
  ...
  $oname=shift;
  $output_pathway_file = $opt_f .'/'. $oname . '_acc_pathway_counts.txt';
  ...
  $output_pathway_file2 = $opt_f .'/'. $oname . '_pathway_counts.txt';
...
sub generate_gaf_output($$$$$$) {
  ...
  $output_gaf_file = $opt_f .'/'. $oname . '_gaf.txt';
